...
\begin{titlepage}
  ...
  \begin{tcolorbox}[casebox,width=0.85\linewidth]
    ...
    cases (case311–case329) demonstrate the corresponding physics.
  \end{tcolorbox}
...
\begin{tcolorbox}[casebox]
  \textbf{Demonstrated by:} all nineteen tutorial cases
  (\caseref{case311}–\caseref{case329}).
...
\begin{tcolorbox}[casebox]
  ...
  \caseref{case327}–\caseref{case329}.
\end{tcolorbox}
...
\begin{tcolorbox}[casebox]
  ...
  \caseref{case329} (FLiBe channel with distributed H$_2$ source and
  SS316 permeation).
\end{tcolorbox}
...
\begin{tcolorbox}[casebox]
  ...
  \caseref{case328}–\caseref{case329}.\\
...
\begin{tcolorbox}[casebox]
  ...
  \caseref{case323} (FLiBe and SS316 walls), \caseref{case329} (FLiBe–SS316 interface).
\end{tcolorbox}
...
\begin{tcolorbox}[notebox]
  ...
  profile matters.  Use \texttt{speciesFluid}.  For $\mathrm{Pe} \gg 100$ in
  engineering contexts where only the bulk flux is needed, a turbulent-effective-$D$
  in \texttt{speciesSolid} is usually sufficient and much cheaper.
\end{tcolorbox}
...
Nineteen tutorial cases cover the full range from pure-diffusion verification
to fully coupled fluid-flow species transport.  Cases 311--322 use
\texttt{speciesSolid} exclusively; cases 323--326 extend \texttt{speciesSolid}
with prescribed convection; cases 327--329 introduce \texttt{speciesFluid}.
...
\caseref{case329} & \texttt{speciesFluid} + \texttt{speciesSolid} &
  FLiBe vertical channel with distributed H$_2$ source $\to$ SS316 permeation.
  Uniform tritium-production source ($S_\text{vol}$); $\mathrm{Pe} = 1000$;
  Sieverts linear interface; vacuum outer BC.
  Demonstrates concentration accumulation and permeation in a molten-salt HX
  channel relevant to fusion breeding blankets. \\
...
\begin{table}[H]
\centering
\caption{Which tutorials exercise which physical models (all 19 cases).
...
Latent heat flux BC (§\ref{sec:latent})
  & & & & & & & & & & & &
  & & & &\checkmark&\checkmark& & \\
Momentum eq.\ PIMPLE~\eqref{eq:mom}
  & & & & & & & & & & & &
  & & & & &\checkmark&\checkmark&\checkmark \\
